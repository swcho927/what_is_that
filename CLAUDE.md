...
firstblood.js    최초 해결 실시간 알림 팝업 (firstSolves 구독, 메인 페이지에 로드)
...
- **개수**: 난이도 비례 — 티어 베이스 **Bronze 50 / Silver 60 / Gold 70 / Platinum 80 / Diamond 90 / Ruby 100**에 **티어 안에서도 레벨별 그라데이션**을 준다. 레벨 오프셋 **V +0 / IV +3 / III +5 / II +8 / I +10** (예: Bronze는 50~60, Silver는 60~70, …). 이건 "대략"이 아니라 **목표치로 맞춘다**(±몇 개 허용)
  - **예외**: 상수 출력처럼 케이스가 의미 없는 문제(1006·1007), 입력 도메인이 목표보다 작은 문제(1014는 1≤N≤50이라 가능한 입력이 50개뿐 → 전 범위 덮으면 끝)
- **크기 분포는 극단값(최대 N) 쪽으로 몰 것**: 채점은 **전 케이스 중 최대 시간**으로 판정되고 작은 케이스는 0ms라 사실상 난이도를 정하는 건 최대 N 케이스다. 그러니 작은/중간 값은 **정답 검증에 필요한 큐레이션 케이스(경계·특이·예제)만** 남기고, **랜덤 대량 생성분은 대부분 최대 N 근처**에 배치한다. 카운트 예산을 중간 크기 랜덤에 낭비하지 말고 최댓값 집중 케이스에 몰 것 (목표 개수의 절반 안팎을 최댓값 근처로 잡는 게 기준)

...
- `아어거` (0번 메모리 접근) 버그: `parseAtom`에서 `어` 토큰을 소비하지 않고 0 반환 (judge.js에 반영 완료)
  ...
  ```
- 입력 파싱: 숫자(`뭐지`)와 문자(`진짜뭐지`)가 **하나의 커서(`inputPos`)를 공유**
  - `뭐지`(`readNum`): 앞쪽 공백·개행 건너뛰고 다음 토큰을 정수로 읽음 (기존과 동일)
  - `진짜뭐지`(`readChar`): **공백·엔터 포함 raw 한 글자씩** 읽음 (ASCII 코드)
  - 입력은 `\r\n`/`\r` → `\n`으로 정규화. 섞여 있어도 커서 공유로 stdin처럼 동작

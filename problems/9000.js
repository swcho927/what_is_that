...
(function () {
    ...
    window.PROBLEMS['9000'] = {
        ...
        description:
            "인천대학교 축제 현장, 가수 한로로가 무대에서 열정적인 공연을 펼치고 있었다. 그런데 갑자기 가수 싸이가 무대 위로 난입하면서 사건이 발생했다. 싸이의 엄청난 에너지 때문에 한로로의 곡 가사 중간중간에 싸이의 강렬한 추임새가 무작위로 끼어들어 가게 된 것이다! 인천대 축제에 가지 못해 방구석에서 아쉬워하던 얼척이는, 이 상황을 컴퓨터로 분석해 보기로 했다. 얼척이의 목표는 원래의 가사 문장과 추임새가 섞인 문장이 주어졌을 때, 섞인 문장에서 불필요한 문자열을 적절히 제거하여 원래의 가사를 복원할 수 있는지 판별하는 프로그램을 만드는 것이다.<br>두 개의 문자열 N과 M이 주어졌을 때, 문자열 N에서 적절히 글자를 제거하여 문자열 M을 만들 수 있는지 판단하는 프로그램을 작성하시오. 단, 글자를 제거할 때 남아있는 문자들의 순서는 유지되어야 한다.",
